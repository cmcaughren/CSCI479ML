"""
   Carolyn McAughren Lab2 CSCI 479 Advanced Topics in Artificial Intelligence, Machine Learning
...
print_state_bool = False
#todo split "best split" into multiple functions for easier reading 
...
def main():
   ...
   #hardcoded for split into 2 bins only
...
def best_split(data, attribute, print_state_bool):
   ...
   #count the number of each item with each Measurement value
   ...
   #count the number of items with 'crit' status, grouped by measurement value
   #create a new dataframe holding these values
   ...
   if print_state_bool:
      print(f"The split value with the lowestt GINI is: {Vo}")
      print("\n")
